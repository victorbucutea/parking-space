...
angular.module('ParkingSpaceMobile.controllers').controller('PaymentsCtrl', function ($rootScope, $state, $scope, offerService, parkingSpaceService, paymentService) {
    ...
    $scope.navigateTo = function () {
        offerService.getOffer($scope.selPayment.order_id, function (data) {
            parkingSpaceService.getSpace(data.parking_space_id, function (data) {
                $state.go('home.map.search', {lat: data.location_lat, lng: data.location_long, zoom: 18});
            })
        });
    };
